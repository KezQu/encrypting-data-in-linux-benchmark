...
{\large \today}\\[5cm]
\includegraphics[width=0.5\textwidth]{PK_logo_with_text.png}\\[1cm]
\vfill
\end{titlepage}
...
zarządzania kluczami dla dm-crypt, zapewniając ustandaryzowany format nagłówka na urządzeniu.
...
\subsubsection{Model ochrony oraz możliwe zagrożenia}
...
takich jak fizyczny dostęp do pamięci urządzenia, zakładając użycie silnego klucza
szyfrującego. Mechanizm ten nie chroni jednak metadanych plików (rozmiary, czy uprawnienia) ani
informacji o strukturze danych. fscrypt nie gwarantuje również ochrony przed manipulacją systemu
plików w trybie offline przed autoryzowanym dostępem użytkownika.
...
mechanizmy kontroli dostępu takie jak uprawnienia plików. Atakujący, który uzyska
...
fscrypt nie zapewnia ochrony danych chronionych aktywnymi kluczami.
...
wybranie sposobu kopiowania plików (sekwencyjne, czy losowe). W ramach benchmarków
przetestowano kopiowanie sekwencyjne i losowe bloków o różnej wielkości oraz
jednoczesne wykonywanie operacji I/O o równym (50/50) oraz nierównomiernym (70/30) rozłożeniu.
...
osiągają zbliżone wyniki, co potwierdza spójność wyników dla tego mechanizmu.
...
Mimo zastosowania sposobów czyszczenia pamięci podręcznej, fscrypt oraz eCrypts wykorzystuje
pamięć RAM, co uniemożliwia wiarygodny pomiar wydajności dyskowej dla tego mechanizmu w
zastosowanej konfiguracji testów.
...
zintegrowany z popularnymi systemami plików takimi jak ext4 i jest aktywnie rozwijany.
...
na poziomie bliskim wyników systemu plików bez szyfrowania. W przypadku eCryptfs odnotowano
wysokie obciążenie CPU (powyżej 80\% sys) oraz znaczne opóźnienia przy operacjach na dużych
blokach. Wyniki fscrypt i eCryptfs w zakresie operacji odczytu są silnie zakłócone przez
buforowanie danych w pamięci RAM, co uniemożliwia wiarygodny pomiar rzeczywistej wydajności
dyskowej dla tych mechanizmów w zastosowanej konfiguracji testów.
\\\\
...
plików (rozmiary, czy uprawnienia). Żaden z analizowanych mechanizmów nie
